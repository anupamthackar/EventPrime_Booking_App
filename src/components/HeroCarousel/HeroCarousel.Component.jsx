    ...
    <>
      ...
      <div className="hidden lg:block">
        <HeroSlider {...settingsLG}>
          {images.map((images, index) => (
            <div className="w-screen h-screen px-1 py-5" key={index}>
              <img
                src={`https://image.tmdb.org/t/p/original${images.backdrop_path}`}
                alt="Hero Banner"
                className="w-screen h-5/6 rounded-md object-cover"
                />
              <div className="bg-blue bg-opacity-20 rounded-md ">
              
                <h3
                  className={`text-2xl pt-2 font-black   ${images.isDark ? "text-white" : "text-blue"
                    }`}
                >
                  🍿&nbsp; {images.original_title}  
                </h3>
                <span
                  className='text-lg   px-10 font-ligh text-blue-600'
                >⭐ {images.vote_average}
                </span>
                <span
                  className='text-lg  pr-10 font-ligh text-red-600'
                >👨🏻 {images.vote_count} 
                </span>
                <span
                  className='text-lg  font-ligh text-green-600'
                >📅 {images.release_date} 
                </span>
              </div>
